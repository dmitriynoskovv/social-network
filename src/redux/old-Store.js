...
// High Order Component - компонент высшего порядка. Принимает на входе компонент и возвращает уже другой компонент
...
// Только в классовой компоненте мы имеем право использовать компоненты жизненного цикла.
// Только в компонентах жизненного цикла мы имеем право использовать сайд эффакты, к которым относятся:
// 1. Асинхронные операции, обращения к серверу
// 2. Сет тайм аут
// 3. Обращение к дом элементам напрямую

// В методе жизненного цикла componentDidUpdate можно использовать сайд эффекты только при заданных условиях, так, как
// обновление стейта при сайд эфектах приведет к постоянному обновлению компонента.

// Реакт имеет встраиваемые в компоненту методы типа setState. Что бы стейт не был разбросан по всему приложению, используется глобальный стейт - Редакс.
